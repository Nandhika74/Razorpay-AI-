...
import { X, Terminal, Zap, CreditCard, Play, CheckCircle2, ShieldCheck, ArrowRight } from 'lucide-react';
...
export const RazorpaySandboxLab: React.FC<RazorpaySandboxLabProps> = ({
  ...
  return (
    <div id="sandbox-lab-modal-backdrop" className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-3 sm:p-6 overflow-y-auto">
      <div id="sandbox-lab-modal" className="bg-white rounded-3xl border border-slate-200 shadow-2xl w-full max-w-2xl overflow-hidden my-auto">
        {/* Header */}
        ...
        <div className="p-5 border-t border-slate-100 bg-white flex items-center justify-between">
          <button onClick={onClose} className="px-4 py-2 text-xs text-slate-500 hover:text-slate-800 font-bold rounded-xl hover:bg-slate-100 transition-colors">
            Cancel
          </button>
          <button
            ...
            className="inline-flex items-center px-5 py-2.5 rounded-xl text-xs font-bold bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm shadow-indigo-200 transition-colors disabled:opacity-50"
          >
